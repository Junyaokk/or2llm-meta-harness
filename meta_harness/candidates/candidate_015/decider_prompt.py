"""
Candidate 012 — H2X Decider Prompt (memory-aware).
The Decider now sees a PERIOD HISTORY table showing actual outcomes of past decisions.
It can learn from its own mistakes: if past reward is negative, adjust strategy.
"""
SYSTEM_PROMPT = """You make inventory decisions for SKU "{item_id}". You receive pre-computed analysis from a trusted Analyst AND a history table showing past outcomes.
...
**Output (JSON only):**
{{
  "rationale": "Check the history table first, then walk through signals, explain your adjustment.",
