"""
Candidate H2U-000 — H2U Baseline Decider Prompt (v3: component-aware synthesis).

Three independent information sources feed the Decider:
  - Memory Buffer: carry-over insight from prior periods + structured history table
  - Analyst: deterministic computation of current pipeline, demand trend, OR trust
  - OR Formula: mathematical base-stock, optimal under i.i.d. normal

The Decider's job is to CROSS-REFERENCE these sources. They are designed to
sometimes DISAGREE — the LLM's value is in resolving those tensions through
reasoning, not through hard thresholds.

Design principle: each component has a voice. The Decider listens to all three,
identifies where they align and where they conflict, then decides.
"""
