"""Evaluate H2X best (candidate_014) on 10 holdout instances."""
...
ALL_10_INSTANCES = [
    ...
    # New 5 — complementary demand patterns
...
cand_dir = Path(__file__).resolve().parent / "candidates" / "candidate_014"
store = TraceStore(Path(__file__).resolve().parent.parent / "traces" / "trace_store_h2x_10inst")
...
t0 = time.time()
result = evaluator.evaluate("candidate_014")
elapsed = time.time() - t0

store.save(result)

print(f"\n{'='*60}")
print(f"H2X (candidate_014) on 10 instances — DONE")
