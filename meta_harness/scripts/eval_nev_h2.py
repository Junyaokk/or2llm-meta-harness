"""Evaluate H2 best (candidate_011) on 10 NEV supply chain instances."""
...
cand_dir = Path(__file__).resolve().parent / "candidates" / "candidate_011"
store = TraceStore(Path(__file__).resolve().parent.parent / "traces" / "trace_store_h2_10nev")
...
t0 = time.time()
result = evaluator.evaluate("candidate_011")
elapsed = time.time() - t0

store.save(result)

print(f"\n{'='*60}")
print(f"H2 BEST (candidate_011) on 10 NEV instances — DONE")
print(f"Mean NR: {result.mean_nr:.4f}  ({elapsed:.0f}s)")
for label, nr in sorted(result.per_instance_nr.items()):
    print(f"  {label}: {nr:.4f}")

summary = {"architecture":"H2","candidate":"candidate_011","mean_nr":result.mean_nr,
