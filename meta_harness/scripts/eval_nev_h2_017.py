"""Evaluate H2 candidate_017 (H1-knowledge-informed) on 10 NEV instances."""
...
cand_dir = Path(__file__).resolve().parent / "candidates" / "candidate_017"
store = TraceStore(Path(__file__).resolve().parent.parent / "traces" / "trace_store_h2_017_10nev")
...
t0 = time.time()
result = evaluator.evaluate("candidate_017")
elapsed = time.time() - t0

store.save(result)

print(f"\n{'='*60}")
print(f"H2 (candidate_017, H1-informed) on 10 NEV instances — DONE")
print(f"Mean NR: {result.mean_nr:.4f}  ({elapsed:.0f}s)")
for label, nr in sorted(result.per_instance_nr.items()):
    print(f"  {label}: {nr:.4f}")

summary = {"architecture":"H2","candidate":"candidate_017","mean_nr":result.mean_nr,
           "per_instance_nr":result.per_instance_nr,"elapsed_s":elapsed,
           "note":"H2 with H1 best (004) knowledge baked into Analyst config + Decider prompt"}
